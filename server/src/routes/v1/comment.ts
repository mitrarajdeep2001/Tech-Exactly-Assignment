...
import updateComment from '@/controllers/v1/comment/update_comment';
...
router.put(
  '/:commentId', // Route to comment a blog
  authenticate, // Middleware to verify if the user is authenticated
  authorize(['admin', 'user']), // Middleware to check if the user has the required role
  param('commentId')
    .notEmpty()
  ...
  body('content').notEmpty().withMessage('Content is required'),
  validationError,
  updateComment, // Controller function that handles the "comment" logic
);
