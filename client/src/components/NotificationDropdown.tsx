...
export default function NotificationDropdown() {
  ...
  useEffect(() => {
    setNotificationCount(unreadCount || user?.notificationCount!);
  }, [unreadCount, user]);
  return (
            ...
            notifications.map((notification) => (
              <li key={notification._id}>
                <DropdownItem
                  ...
                  className={`flex gap-3 my-1 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-200 dark:border-gray-800 dark:hover:bg-white/5 ${notification.isRead ? "" : "bg-gray-100"}`}
                >
                  <span className="relative block w-full h-10 rounded-full z-1 max-w-10">
                    <img
                      ...
                      height={40}
                      src={
                        "https://ui-avatars.com/api/?name=rajdeep&background=random"
                      }
                      alt="User"
                      onError={(e) => {
                        e.currentTarget.src =
